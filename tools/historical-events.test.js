...
const historicalEvents = require("../calendar/data/historical-events.json");

test("historical attendance data has unique valid events and prices", () => {
  assert.equal(historicalEvents.length, 54);
  assert.equal(new Set(historicalEvents.map(event => event.id)).size, historicalEvents.length);
  assert.equal(historicalEvents.filter(event => event.status === "confirmed").length, 53);
  ...
  for (const event of historicalEvents) {
    ...
    assert.match(event.concertDate, /^202[45]-\d{2}-\d{2}$/, `${event.id}: historical date`);
