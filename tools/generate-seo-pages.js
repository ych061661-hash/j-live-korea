...
function checklistMarkup(event) {
  const items = [
    ...
    event.presaleDate ? `선예매 기록은 ${humanDate(event.presaleDate, event.presaleTime)}입니다. 참여 조건과 인증 방법을 먼저 확인하세요.` : `선예매 상태는 ${presaleDisplay(event)}입니다. 팬클럽과 주최사 공지를 함께 확인하세요.`
  ];
