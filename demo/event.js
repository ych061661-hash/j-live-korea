...
function addStructuredData(event) {
  const canonical = location.pathname.includes("/demo/events/")
    ? `${config.siteUrl || location.origin}/demo/events/${encodeURIComponent(event.id)}.html`
    : `${config.siteUrl || location.origin}/demo/event.html?id=${encodeURIComponent(event.id)}`;
...
function renderEvent(event) {
  ...
  const correctionBase = location.pathname.includes("/demo/events/") ? "../corrections.html" : "./corrections.html";
  ...
  photo.src = event.youtubeProfileImage ||
...
async function initializeEvent() {
  ...
  if (!response.ok) {
    const fallback = location.pathname.includes("/demo/events/") ? "../data/events.json" : "./data/events.json";
...
initializeEvent().catch(error => {
  const indexPath = location.pathname.includes("/demo/events/") ? "../index.html" : "./index.html";
