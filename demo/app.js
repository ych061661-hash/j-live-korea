...
function renderDetail(schedule, type) {
  ...
  if (schedule.youtubeProfileImage) {
    photo.src = schedule.youtubeProfileImage;
  } else if (schedule.youtubeChannel) {
    photo.src = `https://unavatar.io/youtube/${encodeURIComponent(schedule.youtubeChannel)}?fallback=false`;
  }
  photo.onerror = () => {
    if (schedule.youtubeChannel && photo.src !== `https://unavatar.io/youtube/${encodeURIComponent(schedule.youtubeChannel)}?fallback=false`) {
      photo.src = `https://unavatar.io/youtube/${encodeURIComponent(schedule.youtubeChannel)}?fallback=false`;
      return;
    }
    photo.hidden = true;
