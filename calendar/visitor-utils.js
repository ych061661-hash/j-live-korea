...
(function exposeVisitorUtils(root) {
  ...
  function ticketStatus(event, now = new Date()) {
    ...
    if (!date || !isVerifiedDate(event, field)) return { key: "unknown", label: "정보 미확인" };
  ...
  function verificationDates(event) {
    return {
      schedule: event.scheduleVerifiedAt || event.verifiedAt || "",
      price: event.priceVerifiedAt || ""
    };
  }

  root.JLIVE_VISITOR = { isVerifiedDate, seoulDateParts, ticketStatus, timeMinutes, verificationDates };
